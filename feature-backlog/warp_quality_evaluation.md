...
Evaluates RoMA warp quality for CO3D image pairs at a given camera distance range. For each pair it computes bidirectional warps, measures pixel-space MSE, and saves visualizations with L2 error heatmaps.
...
|---|---|---|
| `--annotation_path` | `hydrant_train.jgz` | CO3D annotation file |
| `--co3d_root` | `co3d_full` | CO3D images root |
| `--output_dir` | `eval_outputs/warp_quality` | Output directory |
| `--distance_min` | 2.0 | Min camera distance |
| `--distance_max` | 5.0 | Max camera distance |
...
- Per-pair PNG in `eval_outputs/warp_quality/` showing: source, target, warped images, L2 error heatmaps with MSE values and camera distance
...
- RoMA may output warps at a different resolution than `--image_size`; the script handles resizing automatically
