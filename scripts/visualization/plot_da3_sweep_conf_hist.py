"""
Per-bin histogram of DA3 warp confidence from per_pair.csv.

Small multiples: one histogram per distance bin, shared x-axis (confidence
in [0, 1]). Mean + median overlaid as vertical lines.

Usage:
    python scripts/visualization/plot_da3_sweep_conf_hist.py --sweep_dir eval_outputs/da3_distance_sweep
"""
...
def main():
    ...
    for idx, bi in enumerate(bins_sorted):
        ...
        vals = np.array([g["conf_da3"] for g in by_bin[bi]])
        ...
        if idx // ncols == nrows - 1:
            ax.set_xlabel("DA3 warp confidence")
        if idx % ncols == 0:
            ax.set_ylabel("pairs")
    ...
    fig.suptitle("DA3 warp confidence distribution per distance bin", fontsize=11)
    fig.tight_layout()
